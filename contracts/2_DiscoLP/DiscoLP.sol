...
contract DiscoLP is ERC20, Ownable, ReentrancyGuard
{
  ...
  constructor (string memory _name, string memory _symbol, uint8 _decimals, address _reserveToken, address _router)
  ...
  function depositToken(address _token, uint256 _amount, uint256 _minShares) external nonReentrant
  {
    ...
    uint256 _cost = UniswapV2LiquidityPoolAbstraction._joinPool(router, reserveToken, _token, _amount, _minCost);
...
library UniswapV2LiquidityPoolAbstraction
{
  ...
  function _joinPool(address router, address _pair, address _token, uint256 _amount, uint256 _minShares) internal returns (uint256 _shares)
  {
